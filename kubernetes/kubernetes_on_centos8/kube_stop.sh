...
# NOTE: in my case the vagrant installed under D Drive on Windows, so /d/..

# usage:  
# to stop individual system -> sh kube_stop.sh dns or node1 or node2 or node3
# Or to stop all in one go then type on the command line
